...
func pillar() sdf.SDF3 {
	...
	bottomTri, err := triangle(TRIANGLE_LENGTH, TRIANGLE_HEIGHT, offset)
	if err != nil {
		log.Fatal(err)
	}
	topTri, err := triangle(TRIANGLE_LENGTH/2, TRIANGLE_HEIGHT/2, offset)
	if err != nil {
		log.Fatal(err)
	}
...
func support() sdf.SDF3 {
	...
	base = sdf.Union3D(
		base,
		sdf.Transform3D(pillar, sdf.Translate3d(v3.Vec{X: TRIANGLE_LENGTH / 48, Y: 0, Z: INSERT_HEIGHT/2 + PILLAR_HEIGHT/2})),
	)
	return base
}
...
func triangle(length, height, offset float64) (sdf.SDF2, error) {
	...
	dimens := []v2.Vec{ // left to top to right
		...
		{X: x/2 - offset, Y: y},
		{X: x/2 + offset, Y: y},
		{X: x + offset/2, Y: offset - offset/2},
		{X: x - offset/2, Y: -offset + offset/2},
	}
